      ...
      <main data-release-preview-compact-background="" style={pageStyle}>
        <ReleasePreview
          ...
          release={{
            ...
            reason:
              "Every unusually long caller-supplied evidence reference remains visible while this production dossier is reviewed on a narrow viewport."
          }}
          stages={stageSlot}
          workset={worksetSlot}
        />
...
export const CompactForcedColors: Story = {
  ...
  play: async ({ canvas, canvasElement }) => {
    const dialog = canvas.getByRole("dialog", {
      name: "Release preview: v2.4.0 The Deliberately Long Copper Finch Identity"
    })
    const summary = canvasElement.querySelector<HTMLElement>("[data-rly-release-preview-summary]")
    ...
    await expect(canvasElement.querySelectorAll("[data-rly-release-preview-slot]")).toHaveLength(5)
