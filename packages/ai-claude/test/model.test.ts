...
it.effect("interrupts a blocked final response observer at the request deadline", () =>
  Effect.gen(function*() {
    ...
    const fiber = yield* LanguageModel.generateText({ prompt: "Show this request" }).pipe(
      Effect.provide(model({
        cwd: "/workspace",
        timeout: "1 second",
        onActivity: (event) =>
      ...
      Effect.provide(fakeProcessLayer([], { stdout: success("completed") })),
      Effect.forkChild
    )
    ...
    expect(error.reason).toMatchObject({
      _tag: "InternalProviderError",
      metadata: { "claude-cli": { phase: "timeout" } }
    })
