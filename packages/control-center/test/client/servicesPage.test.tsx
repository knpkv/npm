...
import {
  type AwsProfileDiscoveryResponse,
...
describe("ServicesPage connection tests", () => {
  ...
  it("discovers AWS profiles and fills the selected profile region", async () => {
    ...
    const discoverAwsResources = vi.fn().mockResolvedValue({
    ...
    const transport: ConnectionTestTransport = {
      ...
      discoverAtlassianProfiles: () =>
        Promise.resolve([
          ...
          {
            profileId: "account-2@cloud-2",
            name: "Blair Chen @ other.atlassian.net",
    ...
    async ({ connections, heading, providers, route, selectsCompletedProfile }) => {
      ...
      const transport: ConnectionTestTransport = {
        ...
        discoverAtlassianProfiles: () =>
          Promise.resolve([
            {
              profileId: "aaa@cloud-before",
              name: "AAA profile that sorts first",
              siteUrl: "https://first.atlassian.net/",
              cloudId: "cloud-before",
              accountName: "Alex Avery",
  ...
  it("submits only Jira after returning with a profile granted for both Atlassian products", async () => {
    ...
    const host = await renderServices(
      transport,
      `/services?enable=jira&atlassianProfile=${encodeURIComponent(profileId)}&atlassianProvider=jira`
    )
    await act(async () => undefined)
    const submit = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Connect Atlassian")
  ...
  it("requests durable Atlassian scopes when extending a canonical OAuth-backed product", async () => {
    let resolveProfiles: ((profiles: AtlassianProfileDiscoveryResponse) => void) | undefined
    const profiles = new Promise<AtlassianProfileDiscoveryResponse>((resolve) => {
      resolveProfiles = resolve
    })
    const previousLocation = window.location.href
    const authorizationUrl = new URL(`/authorize?state=${grantId}`, previousLocation).href
    const startAtlassianOAuthGrant = vi
      .fn<NonNullable<ConnectionTestTransport["startAtlassianOAuthGrant"]>>()
      .mockResolvedValue({
        _tag: "ready",
    ...
    const host = await renderServices(transport, "/services?enable=jira")
    let signIn = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Sign in with Atlassian")
    )
    ...
    await act(async () =>
      resolveProfiles?.([
        {
          profileId: "account-1@cloud-1",
          name: "Existing Confluence OAuth profile",
          siteUrl: "https://team.atlassian.net/",
          cloudId: "cloud-1",
          accountName: "Avery Bell",
          accountEmail: "avery@example.com",
    ...
    signIn = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Sign in with Atlassian")
    )
  ...
  it("keeps an explicit different-cloud OAuth return product-scoped", async () => {
    ...
    const transport: ConnectionTestTransport = {
      ...
      discoverAtlassianProfiles: () =>
        Promise.resolve([
          ...
          {
            profileId: "account-2@cloud-2",
            name: "Completed Jira profile on another cloud",
            siteUrl: "https://other.atlassian.net/",
            cloudId: "cloud-2",
            accountName: "Blair Chen",
            accountEmail: "blair@example.com",
    ...
    const host = await renderServices(
      transport,
      "/services?enable=jira&atlassianProfile=account-2%40cloud-2&atlassianProvider=jira"
    )
    await act(async () => undefined)
    const signIn = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Sign in with Atlassian")
    ...
    expect(startAtlassianOAuthGrant).toHaveBeenCalledOnce()
    expect(startAtlassianOAuthGrant.mock.calls[0]?.[0]).toEqual(["jira"])
  })
  ...
  it("lets a returned OAuth flow replace stored credentials before any profile is saved", async () => {
    const previousLocation = window.location.href
    const originalAuthorization = new URL("#stored-unverified-oauth", previousLocation)
    originalAuthorization.searchParams.set("state", grantId)
    const replacementAuthorization = new URL("#replacement-oauth", previousLocation)
    replacementAuthorization.searchParams.set("state", "c".repeat(43))
    let startsWithoutConfiguration = 0
    const startAtlassianOAuthGrant = vi.fn<NonNullable<ConnectionTestTransport["startAtlassianOAuthGrant"]>>(
      (_providers, _signal, configuration) => {
        if (configuration !== undefined) {
          return Promise.resolve({
            _tag: "ready",
            authorizationUrl: replacementAuthorization.href,
            callbackUrl: "http://127.0.0.1:4173/services/oauth/atlassian/callback"
          })
        }
        startsWithoutConfiguration += 1
        return Promise.resolve(
          startsWithoutConfiguration === 1
            ? {
                _tag: "ready",
                authorizationUrl: originalAuthorization.href,
                callbackUrl: "http://127.0.0.1:4173/services/oauth/atlassian/callback"
              }
            : {
                _tag: "configuration-required",
    ...
    await act(async () => button("Sign in with Atlassian")?.click())
    expect(window.location.href).toBe(originalAuthorization.href)

    await act(async () => root?.unmount())
    root = undefined
    document.body.replaceChildren()
    window.history.replaceState(null, "", previousLocation)
    const returnedHost = await renderServices(transport, "/services?enable=jira")
    await act(async () => undefined)
    const returnedButton = (text: string): HTMLButtonElement | undefined =>
      [...returnedHost.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
        textContent?.includes(text)
      )
    await act(async () => returnedButton("Sign in with Atlassian")?.click())
    const clientId = returnedHost.querySelector<HTMLInputElement>('input[maxlength="500"]')
    const clientSecret = returnedHost.querySelector<HTMLInputElement>('input[type="password"][maxlength="16384"]')
    expect(clientId).not.toBeNull()
    expect(clientSecret).not.toBeNull()
    if (clientId === null || clientSecret === null) return

    await setControlValue(clientId, "replacement-client")
    await setControlValue(clientSecret, "replacement-secret")
    await act(async () => returnedButton("Save OAuth app and continue")?.click())

    expect(startAtlassianOAuthGrant).toHaveBeenCalledTimes(3)
    expect(startAtlassianOAuthGrant.mock.calls[2]?.[2]).toEqual({
      clientId: "replacement-client",
      clientSecret: "replacement-secret"
    })
    expect(window.location.href).toBe(replacementAuthorization.href)
    window.history.replaceState(null, "", previousLocation)
  })
  ...
  it("cancels a pending OAuth start when switching to API-token mode without blocking a later redirect", async () => {
    ...
    const previousLocation = window.location.href
    const authorizationLocation = new URL("#atlassian-oauth-ready", previousLocation)
    authorizationLocation.searchParams.set("state", grantId)
    const authorizationUrl = authorizationLocation.href
    const startAtlassianOAuthGrant = vi.fn<NonNullable<ConnectionTestTransport["startAtlassianOAuthGrant"]>>(
      (_providers, signal) => {
  ...
  it("preserves a verified manual region when profile discovery finishes later", async () => {
    let resolveProfiles: ((profiles: AwsProfileDiscoveryResponse) => void) | undefined
    const profiles = new Promise<AwsProfileDiscoveryResponse>((resolve) => {
      resolveProfiles = resolve
    })
    const transport: ConnectionTestTransport = {
      create: vi.fn(),
      discoverAwsProfiles: () => profiles,
      discoverAwsResources: () =>
        Promise.resolve({
          accountId: "123456789012",
          codeCommit: { _tag: "available", names: ["payments-api"], truncated: false },
          codePipeline: { _tag: "available", names: [], truncated: false }
        }),
      overview: () => Promise.resolve({ ...overview, connections: [] }),
      makeConnectionId: () => Promise.resolve(connection.pluginConnectionId),
    ...
    await act(async () => undefined)
    const region = host.querySelectorAll<HTMLInputElement>("input")[2]
    if (region !== undefined) await setControlValue(region, "us-east-1")
    const discover = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Test & discover")
    )
    await act(async () => discover?.click())
    const repositoryChoice = [...host.querySelectorAll<HTMLLabelElement>("label")]
      .find(({ textContent }) => textContent?.includes("payments-api"))
      ?.querySelector<HTMLInputElement>('input[type="checkbox"]')
    await act(async () => repositoryChoice?.click())

    await act(async () => resolveProfiles?.([{ profile: "default", region: "eu-west-1" }]))

    expect(region?.value).toBe("us-east-1")
    expect(host.textContent).toContain("Verified AWS account 123456789012")
    expect(repositoryChoice?.checked).toBe(true)
  })

  it("does not submit manual AWS names while their entry panel is hidden", async () => {
    const field = (key: string, defaultValue: string | null = null) => ({
      key,
      label: key,
      description: `Configure ${key}.`,
    ...
    const create = vi.fn<ConnectionTestTransport["create"]>(() => new Promise(() => undefined))
    const transport: ConnectionTestTransport = {
      create,
      discoverAwsResources: () =>
        Promise.resolve({
    ...
    await act(async () => discover?.click())
    const repositoryNames = host.querySelectorAll<HTMLTextAreaElement>("textarea")[0]
    if (repositoryNames !== undefined) await setControlValue(repositoryNames, "manual-repository")
    const pipelineChoice = [...host.querySelectorAll<HTMLLabelElement>("label")]
      .find(({ textContent }) => textContent?.includes("payments-production"))
      ?.querySelector<HTMLInputElement>('input[type="checkbox"]')
    await act(async () => pipelineChoice?.click())
    const hideManualEntry = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Hide manual name entry")
    )
    await act(async () => hideManualEntry?.click())
    const submit = [...host.querySelectorAll<HTMLButtonElement>("button")].find(({ textContent }) =>
      textContent?.includes("Connect AWS account")
    )
    await act(async () => submit?.click())
    ...
    expect(request?.providerId).toBe("codepipeline")
    expect(request?.values.find(({ key }) => key === "pipelineName")?.value).toBe("payments-production")
  })
