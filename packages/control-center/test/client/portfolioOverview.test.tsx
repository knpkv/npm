...
describe("PortfolioOverviewView", () => {
  ...
  it("explains the authenticated empty portfolio without fake metrics", () => {
    ...
    expect(markup).toContain("Choose your first service")
    expect(markup).toContain("Enable and verify one source")
    expect(markup.match(/href="\/services\?enable=/gu)).toHaveLength(5)
    expect(markup).toContain('href="/services?enable=codecommit"')
    expect(markup).toContain('href="/services?enable=codepipeline"')
    expect(markup).toContain('href="/services?enable=jira"')
    expect(markup).toContain('href="/services?enable=confluence"')
    expect(markup).toContain('href="/services?enable=clockify"')
    expect(markup).toContain("CodeCommit")
    expect(markup).toContain("CodePipeline")
    expect(markup).toContain("Jira")
    expect(markup).toContain("Confluence")
    expect(markup).toContain("Clockify")
