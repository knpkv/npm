...
describe("BlobStore", () => {
  ...
  descriptorIt.effect(
    "publishes synced owner-only bytes in the two-level content tree",
    () =>
  ...
  descriptorIt.effect(
    "deduplicates concurrent first writes while creating shared directories",
    () =>
      withBlobStore((store) =>
        Effect.gen(function*() {
  ...
  descriptorIt.effect(
    "syncs the published directory before a concurrent link loser returns",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
  ...
  descriptorIt.effect(
    "deduplicates verified hits, repairs reproducible corruption, and rejects durable replacement",
    () =>
      withBlobStore((store, root) =>
        Effect.gen(function*() {
          const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "finishes replacement durability before honoring repair interruption",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
  ...
  descriptorIt.effect(
    "interrupts temporary repair writes without replacing existing bytes",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
  ...
  descriptorIt.effect(
    "does not resolve a digest through another workspace",
    () =>
      withBlobStore((store) =>
        Effect.gen(function*() {
  ...
  descriptorIt.effect(
    "checks the allocation bound before readAll and verifies returned bytes",
    () =>
      withBlobStore((store, root) =>
        Effect.gen(function*() {
          const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "provides bounded range and streaming reads without claiming whole-object verification",
    () =>
      withBlobStore((store) =>
        Effect.gen(function*() {
  ...
  descriptorIt.effect(
    "pins a lazy stream to the validated inode before reading bytes",
    () =>
      withBlobStore((store, root) =>
        Effect.gen(function*() {
          const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "opens before validation and never returns bytes from a swapped outside path",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
  ...
  descriptorIt.effect(
    "fails a range when the opened descriptor reaches premature EOF",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
        const path = yield* Path.Path
  ...
  descriptorIt.effect(
    "fails a stream when the opened descriptor reaches premature EOF",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "rejects oversized writes and oversized corruption before verification reads",
    () =>
      withBlobStore((store, root) =>
        Effect.gen(function*() {
          const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "rejects oversized and out-of-bounds range reads",
    () =>
      withBlobStore((store) =>
        Effect.gen(function*() {
          const stored = yield* store.put(WORKSPACE_A, encoder.encode("abcdefghijklmnop"), "durable")
  ...
  descriptorIt.effect(
    "rejects a symbolic-link shard before writing outside blobRoot",
    () =>
      withBlobStore((store, root) =>
        Effect.gen(function*() {
          const fs = yield* FileSystem.FileSystem
  ...
  descriptorIt.effect(
    "closes file handles so the object root is removable after use",
    () =>
      Effect.gen(function*() {
        const fs = yield* FileSystem.FileSystem
