...
`makeJiraReadPluginRuntime` from `@knpkv/control-center/server` builds the production Jira adapter around the shared Schema-validated `JiraApiClient`. It negotiates bounded project synchronization, `entity.read` for `jira.issue`, and governed proposal, execution, and reconciliation capabilities for description edits, durable comments, and available workflow transitions. Each action is pinned to the inspected issue revision; stale revisions or unavailable transitions fail before a provider mutation.
...
Comment mutations carry a durable Control Center idempotency property. A timeout after dispatch becomes an unknown outcome and reconciliation searches the bounded comment history for that property, then verifies the exact authorized payload digest before reporting success. Description and transition reconciliation similarly compare current provider state instead of replaying a mutation. Threaded-comment fallback and issue link/fix-version association remain follow-up Jira actions.
