...
The canonical item page presents synchronized safe Markdown as an in-place visual editor for workspace owners. Saving always targets the exact normalized page and revision; Relay can draft against that same page before the owner confirms publication. Markdown task checkboxes on pages related directly to a release are counted in the release workset and can be ticked through the same exact-revision governed update without entering edit mode. Unchecked tasks, lazy page bodies, or a truncated release graph block a new Jira release-version publication because the server cannot prove that the Confluence checklist is complete. The release agent can also select any synchronized page with loaded content as a template, place a copy in the release editor, and create a separate Confluence page. The immutable action metadata retains the normalized template entity identity while the source template remains unchanged.
