...
Catalog field metadata marks adapter settings separately from credential fields. Jira and Confluence prefer one shared local Atlassian OAuth profile. An owner can start PKCE-protected Atlassian sign-in from Services, return to the session-bound callback route, and explicitly choose one accessible site. Control Center writes the selected token to both machine-local CLI profile stores so Jira and Confluence refresh the same identity. Before writing, it rechecks each destination's OAuth client configuration. A populated store with a missing or different client configuration rejects the completion without changing either store, while an empty destination receives the shared configuration and token. The browser sees only the authorization URL, account metadata, accessible sites, and final profile metadata; client secrets and provider tokens never enter SQL or browser state. Existing `jira-cli` and `confluence-to-markdown` profiles remain discoverable, and an explicit API-token fallback retains the `email` and `apiToken` secret-store path.

Create one shared Atlassian OAuth app for Control Center on the server machine. Add `<public-origin>/services/oauth/atlassian/callback` as its callback URL and enable the complete scope union requested by the Jira and Confluence integrations: `read:jira-work`, `write:jira-work`, `read:jira-user`, `manage:jira-project`, `manage:jira-configuration`, `read:page:confluence`, `write:page:confluence`, `delete:page:confluence`, `read:attachment:confluence`, `write:attachment:confluence`, `read:me`, and `offline_access`. Configure that same client ID and secret in both CLI stores:

```sh
jira auth configure --client-id <shared-client-id> --client-secret <shared-client-secret>
confluence auth configure --client-id <shared-client-id> --client-secret <shared-client-secret>
```

Control Center returns `configuration-required` unless both `jira-cli` and `confluence-to-markdown` OAuth configurations are present and exactly equal. Services reports the exact callback URL in that state. The CLI commands persist those configurations under the XDG Atlassian config root for their respective stores. Non-loopback callbacks use the trusted HTTPS proxy or direct-TLS setup described above; insecure LAN mode cannot configure providers. A grant expires after ten minutes, is bound to the initiating workspace and browser session, and is consumed once at code exchange and once at site selection.

Clockify similarly keeps `apiKey` in the secret store, and its API origin remains a fixed server-owned runtime constant. AWS setup treats the local profile as a machine-local credential selector, never as cloud identity. After a successful live test, the server transactionally reuses the discovered AWS account, follows the discovered CodeCommit repository or CodePipeline pipeline, and binds the executable connection; one account can therefore own many independently followed resources. The Services overview presents that ownership directly as one verified account card with compact repository and pipeline rows. The server discovers the shared AWS CLI profile catalogue from its standard AWS files and returns only profile names and configured regions; credentials never cross the API or enter the browser. Manual profile entry remains available when discovery is unavailable. The ordinary `start` command installs the production runtime registry; specialized embedded compositions without a `PluginConnectionMap` retain a visible disabled draft and report service unavailable.
