...
`makeClockifyReadPluginRuntime` from `@knpkv/control-center/server` builds the production Clockify adapter around the shared Schema-validated `ClockifyApiClient`. The current descriptor negotiates `entity.read`, bounded `sync.incremental` snapshots on the `time-entries` stream, and governed `action.propose`, `action.execute`, and `action.reconcile` capabilities. `action.execute` is limited to the Control Center-owned `record-approval` decision, which makes no provider call; `correct-association` remains proposal-only because Clockify exposes no provider-enforced atomic revision or idempotency precondition. Credentials remain in the externally supplied client layer.
...
`correct-association` replaces at most one supported leading Jira marker with the reviewed canonical `[KEY]` marker. It binds the exact normalized source revision and preservation fields, then retains a complete proposed replacement containing the frozen start, end, project, task, tags, custom fields, billable state, supported entry type, and corrected description. Normal synchronization appends successor entity and evidence revisions so relationship inference and rollups converge without rewriting history. Clockify exposes no conditional update or idempotency token for this operation, so execution and reconciliation fail closed before any provider write. A future provider-enforced atomic capability requires a separate reviewed contract before correction execution is enabled.
...
Create one shared Atlassian OAuth app for Control Center on the server machine. Add `<public-origin>/services/oauth/atlassian/callback` as its callback URL. Enable Jira's proposal-only read/identity scopes (`read:jira-work`, `read:jira-user`, `read:me`, `offline_access`) and the Confluence integration's documented scopes (`read:page:confluence`, `write:page:confluence`, `delete:page:confluence`, `read:attachment:confluence`, `write:attachment:confluence`). Do not grant Jira write or project-management scopes to the Control Center flow while Jira mutations remain proposal-only.
