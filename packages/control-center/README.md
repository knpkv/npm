...
`makeJiraReadPluginRuntime` from `@knpkv/control-center/server` builds the production Jira adapter around the shared Schema-validated `JiraApiClient`. It negotiates bounded project synchronization, `entity.read` for `jira.issue`, and revision-inspected governed proposals for comments. A proposal preserves the exact `jira.issue` target identity and records the inspected issue revision. Jira provider writes are unnegotiated: comments, description replacement, and workflow transitions remain disabled until Jira offers an atomic provider revision guard or the product adopts an explicit append-only authorization contract.
...
Comment proposals are useful for review and authorization workflows, but this adapter does not execute or reconcile them. Threaded comments, workflow transitions, description replacement, and issue link/fix-version association likewise remain follow-up Jira actions until their provider mutations can enforce atomic revision preconditions or are governed by an explicit append-only contract.
