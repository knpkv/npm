...
OAuth profiles provide the verified cloud ID used to share one Atlassian site across Jira and Confluence. Jira API-token connections remain usable but standalone because the scoped Jira REST surface does not prove that cloud ID. Confluence verifies the configured cloud ID through its system-information response in both authentication modes. Pre-stability Jira descriptors without `siteId` and `projectId` enter an explicit `plugin-configuration-migration-required` state; recreate those local connections while migrations are intentionally disabled.
