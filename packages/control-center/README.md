...
`makeJiraReadPluginRuntime` from `@knpkv/control-center/server` builds the production Jira adapter around the shared Schema-validated `JiraApiClient`. It negotiates bounded project synchronization, `entity.read` for `jira.issue`, and governed proposal, execution, and reconciliation capabilities for durable comments. Each comment is pinned to the inspected issue revision, and stale revisions fail before a provider mutation. Governed description replacement and workflow transitions remain disabled because Jira Cloud does not expose an atomic revision precondition for those mutations.
...
Comment mutations carry a durable Control Center idempotency property. A timeout after dispatch becomes an unknown outcome and reconciliation searches newest-first bounded comment history for that property, then verifies the exact authorized payload digest before reporting success. Threaded-comment fallback, workflow transitions, description replacement, and issue link/fix-version association remain follow-up Jira actions until their provider mutations can enforce atomic revision preconditions.
