...
/** Bounded application failure while producing a canonical readiness digest. */
export class ReadinessDigestError extends Schema.TaggedErrorClass<ReadinessDigestError>()(
...
const digestCanonicalJson = Effect.fn("ReadinessDigests.digestCanonicalJson")(function*(value: string) {
  ...
  ).pipe(Effect.mapError(() => new ReadinessDigestError({ operation: "encode-utf8" })))
  const digest = yield* cryptoService.digest("SHA-256", bytes).pipe(
    Effect.mapError(() => new ReadinessDigestError({ operation: "digest" }))
  )
...
export const digestEnvironmentReadinessCandidate = Effect.fn(
  "ReadinessDigests.environmentCandidate"
)(function*(material: EnvironmentReadinessCandidateMaterial) {
  const value = yield* encodeEnvironmentMaterial(
    normalizeEnvironmentReadinessCandidateMaterial(material)
  ).pipe(Effect.mapError(() => new ReadinessDigestError({ operation: "encode" })))
  return yield* digestCanonicalJson(value)
})

/** Hash the exact set of child assessments forming a release candidate. */
export const digestReleaseReadinessCandidate = Effect.fn(
  "ReadinessDigests.releaseCandidate"
)(function*(material: ReleaseReadinessCandidateMaterial) {
  const value = yield* encodeReleaseMaterial(
    normalizeReleaseReadinessCandidateMaterial(material)
  ).pipe(Effect.mapError(() => new ReadinessDigestError({ operation: "encode" })))
  return yield* digestCanonicalJson(value)
})

/** Hash a complete immutable readiness-rule snapshot. */
export const digestReadinessRule = Effect.fn("ReadinessDigests.rule")(function*(
