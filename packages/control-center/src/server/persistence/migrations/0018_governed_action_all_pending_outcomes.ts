...
/** Require every durable provider outcome to fold before another recovery claim may take ownership. */
export const migration0018GovernedActionAllPendingOutcomes = Effect.gen(function*() {
  const sql = yield* SqlClient.SqlClient

  yield* sql`DROP TRIGGER governed_action_recovery_claim_after_pending_outcome`
  yield* sql`CREATE TRIGGER governed_action_recovery_claim_after_pending_outcome
    ...
    BEGIN
      SELECT RAISE(ABORT, 'governed action pending provider outcome must fold before recovery');
    END`
})
