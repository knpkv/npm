/**
 * Shared Effect Schema definitions for persisted OAuth credentials and site metadata.
 *
 * **Mental model**
 *
 * - **Storage schemas**: {@link OAuthTokenSchema} and {@link OAuthConfigSchema} define the
 *   JSON shape written to `auth.json` / `oauth.json` on disk. They are the single source
 *   of truth for token and config structure.
 * - **Type derivation**: All companion types (`OAuthToken`, `OAuthConfig`, etc.) are derived
 *   from schemas — never duplicated.
 *
 * @module
 */
