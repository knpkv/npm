/**
 * AST-to-Markdown serializer with roundtrip comment encoding for non-Markdown features.
 *
 * **Mental model**
 *
 * - **Visitor over DocumentNode union**: {@link serializeToMarkdown} walks the AST, dispatching
 *   on `_tag` to emit Markdown. Effect-based for composability with the rest of the pipeline.
 * - **Roundtrip via HTML comments**: Confluence-specific data (panel types, task status,
 *   image alignment) is encoded in `<!-- ... -->` comments with base64/URL encoding so
 *   a future parser can reconstruct the original.
 *
 * **Common tasks**
 *
 * - Serialize a document: {@link serializeToMarkdown}
 * - Serialize inline nodes only: {@link serializeInlineNodes}
 *
 * **Gotchas**
 *
 * - Comment-encoded data is fragile if users hand-edit the Markdown — edits inside
 *   `<!-- -->` blocks will break roundtripping.
 *
 * @module
 */
