/**
 * Inline-level AST nodes for text content (emphasis, links, mentions, etc.).
 *
 * **Mental model**
 *
 * - **Leaf and recursive nodes**: `Text`, `InlineCode`, `LineBreak` are leaves;
 *   `Strong`, `Emphasis`, `Highlight` contain nested `InlineNode` children via `Schema.suspend`.
 * - **Unsupported preservation**: {@link UnsupportedInline} captures raw content that can't be
 *   mapped, enabling non-destructive roundtrips.
 *
 * @module
 */
...
export class Strong extends Schema.TaggedClass<Strong>()("Strong", {
  children: Schema.suspend((): Schema.Schema<ReadonlyArray<InlineNode>> => Schema.Array(InlineNode))
}) {}
