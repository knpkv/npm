...
const uploadSvgAttachmentToPage = (filePath: string, pageId: string) =>
  Effect.gen(function*() {
    ...
    const parsed = JSON.parse(output.trim()) as {
      readonly attachment?: { readonly id?: string; readonly filename?: string; readonly mediaType?: string | null }
      readonly inserted?: boolean
    }

    expect(parsed.inserted).toBe(true)
    expect(parsed.attachment?.id).toBeTruthy()
    expect(parsed.attachment?.filename).toBe(INTEGRATION_ATTACHMENT_FILENAME)
