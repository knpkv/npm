 ...
 * **Gotchas**
 *
 * - The settle deadline follows the source evidence, not the displayed allocation. Sequential
 *   allocations can end before the overlapping source cluster that sized them. Every allocation in
 *   that cluster waits for the cluster's last source instant plus transcript-write grace.
...
export const decideWatchWrites = (
  ...
  for (const proposal of proposals) {
    ...
    const settlesAtMs = proposal.settlementEndMs + SETTLE_GRACE_SECONDS * 1000
