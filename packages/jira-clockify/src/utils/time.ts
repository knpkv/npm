...
/** One calendar-day slice of a half-open interval. */
...
/** Split a half-open interval at every local midnight, preserving its elapsed seconds. */
export function splitIntervalByLocalDay(startMs: number, endMs: number): ReadonlyArray<LocalDaySlice> {
  if (endMs <= startMs) {
    return [{ day: localDay(new Date(startMs)), endMs: startMs, seconds: 0, startMs }]
  }
  const slices: Array<LocalDaySlice> = []
  let cursor = startMs
  while (cursor < endMs) {
