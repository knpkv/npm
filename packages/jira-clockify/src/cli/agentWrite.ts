...
/**
 * Where a written entry says it came from. This prose is never load-bearing: ordinary writes tally
 * by `[KEY]` and day. Session-backed writes also carry a source suffix for legacy migration; the
 * private provider-ID ledger, not editable text, keeps corrected-ticket consumption after a write.
 */
...
export type SideOutcome =
  | { readonly _tag: "Written"; readonly seconds: number; readonly segments?: ReadonlyArray<WrittenSegment> }
  | {
    readonly _tag: "PartiallyWritten"
    readonly seconds: number
    readonly segments?: ReadonlyArray<WrittenSegment>
    readonly failure: { readonly _tag: "Refused"; readonly message: string } | { readonly _tag: "NotLoggedIn" }
  }
...
export const writeOutcomeLines = (outcome: WriteOutcome): ReadonlyArray<string> => {
  ...
  if (outcome.clockify._tag === "PartiallyWritten") {
    lines.push(`✓ created ${formatDuration(outcome.clockify.seconds)} in Clockify`)
    lines.push(
      outcome.clockify.failure._tag === "NotLoggedIn"
        ? "✗ Clockify is not logged in"
        : `✗ Clockify: ${outcome.clockify.failure.message}`
    )
  }
  ...
  if (outcome.jira._tag === "PartiallyWritten") {
    lines.push(`✓ posted ${formatDuration(outcome.jira.seconds)} to Jira`)
    lines.push(
      outcome.jira.failure._tag === "NotLoggedIn"
        ? `✗ ${NOT_LOGGED_IN_HINT}`
        : `✗ Jira: ${outcome.jira.failure.message}`
    )
  }
  ...
  Effect.gen(function*() {
    ...
    if (targets.clockify) {
      for (const segment of plan.clockify.segments) {
        ...
        if (result !== null) {
          clockifyFailure = result
          break
        }
        clockifyWrittenSeconds += segment.seconds
        clockifySegments.push({
          startMs: segment.startedAt.getTime(),
          endMs: segment.startedAt.getTime() + segment.seconds * 1000
        })
      }
    ...
    const clockify: SideOutcome = !targets.clockify
      ...
      ? {
        ...
        seconds: clockifyWrittenSeconds,
        segments: clockifySegments,
        failure: { _tag: "Refused", message: clockifyFailure }
      }
    ...
    for (const segment of plan.jira.segments) {
      ...
      jiraWrittenSeconds += segment.seconds
      jiraSegments.push({
        startMs: segment.startedAt.getTime(),
        endMs: segment.startedAt.getTime() + segment.seconds * 1000
      })
    ...
    const jira: SideOutcome = jiraWrittenSeconds > 0 && jiraFailure !== null
      ...
      ? { _tag: "Written", seconds: jiraWrittenSeconds, segments: jiraSegments }
      : jiraFailure ?? { _tag: "Refused", message: "the worklog was not created" }
    return { clockify, jira }
  })

/** Execute CLI/watch proposals while preserving exact corrected-ticket source consumption. */
export const applyProposal = (
  ...
  const prepared = prepareProposal({
    ...
    request: {},
    targets,
    consumed: (block, source) => {
      const corrected = source === "clockify"
        ? block.clockifyConsumedSeconds ?? 0
        : block.jiraConsumedSeconds ?? 0
      const ordinary = SourceConsumption.recordedSecondsInBlock(
        block,
        proposal.recordedIntervals ?? [],
        source
      )
      return Math.min(block.seconds, ordinary + corrected)
    }
  })
  if (prepared._tag !== "Prepared") {
    return applyPlannedWrite(
      service,
      {
        _tag: "Write",
        ticketKey: proposal.ticketKey,
  ...
  const planned = prepared.plan([{
    ticketKey: proposal.ticketKey,
    day: proposal.day,
    clockifySeconds: proposal.clockifySeconds,
    jiraSeconds: proposal.jiraSeconds,
    intervals: proposal.recordedIntervals ?? []
  }])
  if (planned._tag === "Write") {
    const cap = (
      side: PlannedWrite["clockify"],
      requested: number,
      minimumSegmentSeconds = 0
    ): PlannedWrite["clockify"] => {
      let remaining = requested
      const segments = side.segments.flatMap((segment) => {
        const seconds = Math.min(segment.seconds, remaining)
        if (seconds < minimumSegmentSeconds) return []
        remaining -= seconds
        return seconds <= 0 ? [] : [{ ...segment, seconds }]
      })
    ...
    const sourceMarked = (side: PlannedWrite["clockify"]): PlannedWrite["clockify"] => ({
      ...side,
      segments: side.segments.map((segment) => ({
        ...segment,
        description: `${description}\n${
          SourceConsumption.marker(
            `${proposal.day}:${proposal.ticketKey}`,
            segment.block.sourceStartMs ?? segment.block.startMs
          )
        }`
      }))
    })
    return applyPlannedWrite(
      ...
      {
        ...planned,
        clockify: sourceMarked(cap(planned.clockify, proposal.clockifyDelta)),
  ...
  return applyPlannedWrite(
    service,
    {
      ...
      jira: {
        seconds: 0,
        withheldSeconds: belowMinimum ? proposal.jiraDelta : 0,
        segments: [],
        startedAt: undefined
      }
    },
