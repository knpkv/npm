...
export interface IssueFact {
  ...
  /** Assignee display name, or null when Jira reports none or omits it. */
  readonly assignee: string | null
  /** Account-relative assignment, independent of the optional display name. */
  readonly assignment: "mine" | "other-assignee" | "unassigned" | "unknown"
  /** True for me, false for a known other or no owner, null when Jira omits the identity. */
...
/** The configured rule and known Jira facts used to admit an attributed ticket. */
...
/** Admit unknown tickets, but withhold a positive other-owner fact unless explicitly overridden. */
export const isOwnedByMe = (ticketKey: string, policy: OwnershipPolicy): boolean => {
  if (policy.mode === "any" || policy.overrides.includes(ticketKey)) return true
  const fact = policy.facts.get(ticketKey)
  return fact === undefined || fact.mine !== false
}
 ...
 *
 * `accountId` and `cloudId` are stored beside the facts because `mine` is a claim about one account
 * on one Jira site. Switching either makes every stored claim meaningless rather than merely stale,
 * so the whole file is discarded when the identity disagrees.
 */
...
export const emptyCache: IssueCache = { accountId: null, cloudId: null, issues: new Map() }
...
const assignment = (
  assignee: Schema.Json | undefined,
  myAccountId: string | null
): IssueFact["assignment"] => {
  if (assignee === null) return "unassigned"
  if (!isJsonObject(assignee) || myAccountId === null) return "unknown"
  const accountId = assignee["accountId"]
  if (!Predicate.isString(accountId)) return "unknown"
  return accountId === myAccountId ? "mine" : "other-assignee"
}

const ownership = (assignment: IssueFact["assignment"]): IssueFact["mine"] =>
  assignment === "mine" ? true : assignment === "unknown" ? null : false
...
export const factsFromIssues = (
  ...
  for (const issue of issues) {
    ...
    const assigned = assignment(rawAssignee, myAccountId)
    facts.set(key, {
      assignee: nested(fields, "assignee", "displayName"),
      assignment: assigned,
      key,
      mine: ownership(assigned),
...
export const parseIssueCache = (content: string): IssueCache => {
  ...
  const cloudId = Predicate.isString(parsed.cloudId) ? parsed.cloudId : null
  ...
  if (Predicate.isObject(stored)) {
    for (const [key, entry] of Object.entries(stored)) {
      ...
      if (
        !Predicate.isNumber(entry.checkedAtMs) ||
        (entry.mine !== null && !Predicate.isBoolean(entry.mine)) ||
        (entry.assignment !== "mine" && entry.assignment !== "other-assignee" &&
          entry.assignment !== "unassigned" && entry.assignment !== "unknown") ||
        entry.mine !== ownership(entry.assignment)
      ) continue
      issues.set(key, {
  ...
  return { accountId, cloudId, issues }
}
...
export const mergeCache = (options: {
  ...
  readonly accountId: string | null
  readonly cloudId: string | null
  readonly queriedKeys: ReadonlyArray<string>
  readonly nowMs: number
}): IssueCache => {
  // A different account makes every stored `mine` a claim about somebody else.
  const sameIdentity = options.cache.accountId === options.accountId && options.cache.cloudId === options.cloudId
  const queried = new Set(options.queriedKeys)
  const kept = sameIdentity
    ? [...options.cache.issues.entries()].filter(([key]) => !queried.has(key))
    : []
  ...
  return { accountId: options.accountId, cloudId: options.cloudId, issues }
}
    ...
    return IssueFacts.of({
      lookup: (keys) =>
        Effect.gen(function*() {
          ...
          // `mine` is account-relative. Even a fresh cache belongs to nobody until the current
          // identity has been verified, and a cache for another account contributes no facts.
          const scopedCache = cache.accountId === mine && cache.cloudId === cloudId ? cache : emptyCache
          const nowMs = yield* Clock.currentTimeMillis
          const stale = staleKeys({ cache: scopedCache, keys: wanted, nowMs, ttlMs: OWNERSHIP_TTL_MS })
          if (stale.length === 0) return { checked: true, facts: factsOf(scopedCache) }
