...
describe("resolvePeriod", () => {
  ...
  it("starts seven calendar days across spring-forward", () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date(2026, 2, 10, 12))
    const period = resolvePeriod({ week: true, since: undefined, until: undefined })
    expect("error" in period).toBe(false)
    if ("error" in period) return
    expect(localDay(period.from)).toBe("2026-03-04")
    expect(localDay(new Date(period.to.getTime() - 1))).toBe("2026-03-10")
  })
})
