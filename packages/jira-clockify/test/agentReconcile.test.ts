...
const HISTORICAL_NOW = at(2026, 7, 3, 0, 0)

const baseOptions = (overrides: FakeHeadlessOptions = {}): FakeHeadlessOptions => ({
  ...overrides,
  config: { sessionRoots: [WORK_ROOT], sessionOwnership: "any", ...overrides.config }
})

const run = (args: ReadonlyArray<string>, options: FakeHeadlessOptions = {}, nowMs = HISTORICAL_NOW) => {
  const fake = makeFakeHeadless(options)
  return TestClock.setTime(nowMs).pipe(
    Effect.andThen(Command.runWith(root, { version: "0.0.0-test" })(args)),
    Effect.exit,
...
const visibleDescription = (description: string): string => description.replace(/\n\[jcf-source:[^\]]+\]$/, "")
...
const ReportProposal = Schema.Struct({
  ...
  activeSeconds: Schema.optional(Schema.Finite),
  clockifyDelta: Schema.optional(Schema.Finite),
  jiraDelta: Schema.optional(Schema.Finite),
  summary: Schema.NullOr(Schema.String),
  assignee: Schema.NullOr(Schema.String),
  startedAt: Schema.NullOr(Schema.String),
  endedAt: Schema.NullOr(Schema.String),
  blocks: Schema.Array(Schema.Struct({ startMs: Schema.Finite, endMs: Schema.Finite, seconds: Schema.Finite }))
})
...
const AgentReport = Schema.Struct({
  ...
  ownershipWithheld: Schema.Array(
    Schema.Struct({
      ...
      reason: Schema.Literals(["other-assignee", "unassigned"])
    })
  )
})
...
describe("jcf sync reconcile --agent: proposals", () => {
  ...
  it.effect("anchors ordinary provider time after corrected source consumption", () =>
    Effect.gen(function*() {
      ...
      const afternoon = at(DAY.year, DAY.month, DAY.day, 15, 0)
      const sourceMarker = SourceConsumption.marker("2026-07-01:PROJ-1", morning)
      const { world } = yield* run(
        agent(),
        baseOptions({
          transcripts: {
            "work-morning/s-morning.jsonl": transcript({
              sessionId: "s-morning",
              cwd: `${WORK_ROOT}/morning`,
              gitBranch: "feat/PROJ-1-morning",
              events: steady(morning, 55)
            }),
            "work-afternoon/s-afternoon.jsonl": transcript({
              sessionId: "s-afternoon",
              cwd: `${WORK_ROOT}/afternoon`,
              gitBranch: "feat/PROJ-1-afternoon",
              events: steady(afternoon, 55)
            })
          },
          clockifyEntries: [
            {
              id: "corrected-clockify",
              description: `[PROJ-2] corrected\n${sourceMarker}`,
              start: iso(morning),
              end: iso(morning + 3600000)
            },
            {
              description: "[PROJ-1] partial",
              start: iso(afternoon),
              end: iso(afternoon + 1800000)
            }
          ],
          jiraWorklogs: {
            "PROJ-1": [{ started: iso(afternoon), timeSpentSeconds: 1800 }],
            "PROJ-2": [{
              id: "corrected-jira",
              comment: `corrected\n${sourceMarker}`,
              started: iso(morning),
      ...
      expect(world.createdClockifyEntries).toHaveLength(1)
      expect(new Date(world.createdClockifyEntries[0]!.start).getTime()).toBe(afternoon + 1800000)
      expect(world.createdClockifyEntries[0]!.end).toBe(iso(afternoon + 3600000))
      expect(world.jiraWorklogs).toHaveLength(1)
      expect(new Date(world.jiraWorklogs[0]!.started.replace("+0000", "Z")).getTime()).toBe(afternoon + 1800000)
      expect(world.jiraWorklogs[0]!.timeSpentSeconds).toBe(1800)
    }))

  it.effect("never writes more than the provider gaps approved in the picker", () =>
    Effect.gen(function*() {
      const startMs = at(DAY.year, DAY.month, DAY.day, 10, 0)
      const endMs = startMs + 1800000
      const { world } = yield* run(
        agent(),
        baseOptions({
          transcripts: branchSession({ minutes: 60 }),
          clockifyEntries: [{ description: "[PROJ-5662] existing", start: iso(startMs), end: iso(endMs) }],
          jiraWorklogs: { "PROJ-5662": [{ started: iso(startMs), timeSpentSeconds: 1800 }] },
          keep: [true],
          beforeDescribe: (world) => {
            world.setClockifyEntries([])
            world.setJiraWorklogs({})
          }
        })
      )
    ...
    Effect.gen(function*() {
      ...
      const { world } = yield* run(
        ...
        baseOptions({
          ...
          beforeDescribe: (world) => {
            world.setClockifyEntries([
              { description: "[PROJ-5662] appeared", start: iso(startMs), end: iso(endMs) }
            ])
            world.setJiraWorklogs({ "PROJ-5662": [{ started: iso(startMs), timeSpentSeconds: 2700 }] })
          }
        })
      )
      ...
      expect(world.jiraWorklogs).toMatchObject([{ timeSpentSeconds: 1200 }])
    }))

  it.effect("refuses provider writes while another machine writer holds the guard", () =>
    Effect.gen(function*() {
      const fake = makeFakeHeadless(baseOptions({ transcripts: branchSession(), keep: [true] }))
      ...
      expect(output(fake.world.stdout)).toContain("another JCF writer holds the machine guard")

      delete fake.world.writtenFiles[leasePath]
  ...
  it.effect("credits trailing presence only through the current clock", () =>
    Effect.gen(function*() {
      const startedAt = at(DAY.year, DAY.month, DAY.day, 10, 0)
      const options = baseOptions({
        transcripts: {
          "work-repo/s-current.jsonl": transcript({
            sessionId: "s-current",
            cwd: `${WORK_ROOT}/repo`,
            gitBranch: "feat/PROJ-1-current",
            events: steady(startedAt, 0)
          })
        }
      })

      const recent = yield* run(agent(["--json"]), options, startedAt + 60000)
      expect(jsonProposals(recent.world.stdout)[0]?.sessionSeconds).toBe(60)

      const historical = yield* run(agent(["--json"]), options, startedAt + 600000)
  ...
  // load-bearing if a later saved-entry edit changes the ticket; the visible prose is not.
  it.effect("records that the write came from an Agent Session, on both sides", () =>
    Effect.gen(function*() {
      ...
      expect(visibleDescription(description)).toBe("[PROJ-5662] Reconciled from Claude Agent Session")
      expect(SourceConsumption.markers(description)).toHaveLength(1)
      expect(SourceConsumption.markers(world.jiraWorklogs[0]!.comment)).toEqual(
        SourceConsumption.markers(description)
      )
  ...
  describe("entry text", () => {
    ...
    it.effect("still writes when the note cannot be produced", () =>
      Effect.gen(function*() {
        ...
        expect(world.createdClockifyEntries).toHaveLength(1)
        expect(visibleDescription(world.createdClockifyEntries[0]!.description)).toBe(
    ...
    it.effect("falls back to provenance alone when Jira knows neither title nor note", () =>
      Effect.gen(function*() {
        ...
        expect(visibleDescription(world.createdClockifyEntries[0]!.description))
          .toBe("[PROJ-5662] Reconciled from Claude Agent Session")
      }))
  })
  ...
  it.effect("shows the assignee next to the summary", () =>
    Effect.gen(function*() {
      const { world } = yield* run(
        ...
        baseOptions({
          transcripts: branchSession(),
          config: { sessionRoots: [WORK_ROOT], sessionOwnership: "any" },
  ...
  it.effect("does not call a hidden-name other assignee unassigned in the human report", () =>
    Effect.gen(function*() {
      const { world } = yield* run(
        agent(),
        baseOptions({
          ...
          transcripts: {
            "work-repo/s1.jsonl": transcript({
              ...
              cwd: `${WORK_ROOT}/repo`,
              gitBranch: "feat/PROJ-5662-otel",
              events: steady(at(DAY.year, DAY.month, DAY.day, 10, 0), 55)
            })
          },
          issueSummaries: { "PROJ-5662": "Synthetic issue" },
          issueAssignees: { "PROJ-5662": "acct-other" },
          issueAssigneeDisplayNames: { "PROJ-5662": null },
          keep: [false]
        })
      )
      const printed = output(world.stdout)
      expect(printed).toContain("not offered — assigned to somebody else")
      expect(printed).toContain("1h 0m")
      expect(printed).toContain("Synthetic issue")
      expect(printed).not.toContain("unassigned")
      expect(world.createdClockifyEntries).toEqual([])
      expect(world.jiraWorklogs).toEqual([])
    }))
  ...
  for (
    const policy of [
      { label: "my hidden-name assignment", assignee: "acct-me" },
      { label: "omitted assignment identity", assignee: undefined }
    ] satisfies ReadonlyArray<{ readonly label: string; readonly assignee: string | undefined }>
  ) {
    it.effect(`does not invent an unassigned picker label for ${policy.label}`, () =>
      Effect.gen(function*() {
        const { world } = yield* run(
          agent(),
          baseOptions({
            ...
            transcripts: branchSession(),
            issueSummaries: { "PROJ-5662": "Add OTEL spans" },
            issueAssignees: policy.assignee === undefined ? {} : { "PROJ-5662": policy.assignee },
            issueAssigneeDisplayNames: policy.assignee === undefined ? {} : { "PROJ-5662": null },
            keep: [false]
          })
        )
        const picker = output(world.prompts)
        expect(picker).toContain("Add OTEL spans")
        expect(picker).not.toContain("unassigned")
        expect(world.createdClockifyEntries).toEqual([])
        expect(world.jiraWorklogs).toEqual([])
      }))
  }
  ...
  for (
    const policy of [
      ...
      { label: "an explicit ownership override", config: { sessionOwnershipOverrides: ["PROJ-5662"] }, writes: true }
    ...
    it.effect(`${policy.label} controls interactive agent writes for somebody else's ticket`, () =>
      Effect.gen(function*() {
        const { world } = yield* run(
          agent(),
          baseOptions({
            ...
            transcripts: branchSession(),
            issueSummaries: { "PROJ-5662": "Add OTEL spans" },
            issueAssignees: { "PROJ-5662": "someone-else" },
            keep: [true]
          })
        )
        ...
        if (!policy.writes) expect(output(world.stdout)).toContain("not offered — assigned to someone-else")
      }))
  }
  ...
  it.effect("says so when an issue is unassigned", () =>
    Effect.gen(function*() {
      const { world } = yield* run(
        agent(),
        baseOptions({
          config: { sessionOwnership: "assigned" },
          transcripts: branchSession(),
          issueSummaries: { "PROJ-5662": "Add OTEL spans" },
          issueAssignees: { "PROJ-5662": null },
          keep: [false]
        })
      )
      expect(output(world.stdout)).toContain("not offered — unassigned")
      expect(output(world.stdout)).toContain("Add OTEL spans  · unassigned")
      expect(world.createdClockifyEntries).toEqual([])
      expect(world.jiraWorklogs).toEqual([])
    }))
  ...
  it.effect("names the bounds and the blocks inside them when work was interrupted", () =>
    Effect.gen(function*() {
      ...
      expect(row?.blocks).toEqual([
        {
          startMs: at(DAY.year, DAY.month, DAY.day, 9, 0),
          endMs: at(DAY.year, DAY.month, DAY.day, 9, 15),
          seconds: 900
        },
        {
          startMs: at(DAY.year, DAY.month, DAY.day, 11, 0),
          endMs: at(DAY.year, DAY.month, DAY.day, 11, 15),
          seconds: 900
        }
      ])
    }))
    ...
    Effect.gen(function*() {
      const { world } = yield* run(
        ...
        baseOptions({
          ...
          jiraWorklogs: {
            "PROJ-5662": [{ started: iso(at(2026, 6, 30, 23, 30)), timeSpentSeconds: 3900 }]
          },
          keep: [true, true]
        })
      )
  ...
  it.effect("reads the prior start of a Jira worklog that overlaps a narrow day", () =>
    Effect.gen(function*() {
      const startMs = at(2026, 6, 30, 23, 30)
      const { world } = yield* run(
        ["sync", "reconcile", "clockify-to-jira", "--since", "2026-07-01", "--until", "2026-07-01"],
        baseOptions({
          clockifyEntries: [{
            description: "[PROJ-5662] overnight",
            start: iso(startMs),
            end: iso(startMs + 3600000)
          }],
          jiraWorklogs: { "PROJ-5662": [{ started: iso(startMs), timeSpentSeconds: 3600 }] },
          keep: [true]
        })
      )

      expect(world.jiraWorklogs).toEqual([])
      expect(output(world.stdout)).toContain("Clockify 30m 0s  Jira 30m 0s")
    }))

  it.effect("does not pull a non-overlapping prior-day Jira worklog into a narrow day", () =>
    Effect.gen(function*() {
      const { world } = yield* run(
        ["sync", "reconcile", "jira-to-clockify", "--since", "2026-07-01", "--until", "2026-07-01"],
        baseOptions({
          jiraWorklogs: {
            "PROJ-5662": [{ started: iso(at(2026, 6, 30, 22, 0)), timeSpentSeconds: 3600 }]
          },
          keep: [true]
        })
      )
      ...
      expect(world.createdClockifyEntries).toEqual([])
      expect(output(world.stdout)).toContain("No time logged on either side")
    }))
...
describe("jcf sync reconcile <direction>", () => {
  ...
  for (const direction of directions) {
    it.effect(`does not duplicate matching overnight records in ${direction}`, () =>
      Effect.gen(function*() {
        const startMs = at(2026, 6, 30, 23, 30)
        const { world } = yield* run(
          ["sync", "reconcile", direction, "--since", "2026-06-30", "--until", "2026-07-01"],
          baseOptions({
            clockifyEntries: [{
              description: "[PROJ-9] overnight",
              start: iso(startMs),
              end: iso(startMs + 3600000)
            }],
            jiraWorklogs: {
              "PROJ-9": [{ started: iso(startMs), timeSpentSeconds: 3600 }]
            },
            keep: [true]
          })
        )
        ...
        expect(world.createdClockifyEntries).toEqual([])
        expect(world.jiraWorklogs).toEqual([])
        expect(output(world.stdout)).not.toContain("is short by")
      }))

    it.effect(`refuses ${direction} writes while another machine writer holds the guard`, () =>
      Effect.gen(function*() {
        const sourceIsClockify = direction === "clockify-to-jira"
        const fake = makeFakeHeadless(baseOptions({
          clockifyEntries: sourceIsClockify
            ? [{
              description: "[PROJ-9] approved gap",
        ...
        yield* Command.runWith(root, { version: "0.0.0-test" })(["sync", "reconcile", direction, ...SINCE]).pipe(
          Effect.exit,
          Effect.provide(fake.layer)
        )
    ...
    it.effect(`re-tallies and caps an approved ${direction} write inside the guard`, () =>
      Effect.gen(function*() {
        const sourceIsClockify = direction === "clockify-to-jira"
        const startMs = at(DAY.year, DAY.month, DAY.day, 12, 0)
        const fake = makeFakeHeadless(baseOptions({
          clockifyEntries: sourceIsClockify
            ? [{ description: "[PROJ-9] approved gap", start: iso(startMs), end: iso(startMs + 3600000) }]
            : [],
          jiraWorklogs: sourceIsClockify
            ? {}
            : { "PROJ-9": [{ started: iso(startMs), timeSpentSeconds: 3600 }] },
          ...
          beforePromptInput: (world) => {
            if (sourceIsClockify) {
              world.setJiraWorklogs({ "PROJ-9": [{ started: iso(startMs), timeSpentSeconds: 2700 }] })
            } else {
              world.setClockifyEntries([{
                description: "[PROJ-9] appeared",
                start: iso(startMs),
                end: iso(startMs + 2700000)
              }])
            }
          }
        }))
        ...
        if (sourceIsClockify) {
          expect(fake.world.jiraWorklogs).toMatchObject([{ timeSpentSeconds: 900 }])
          expect(fake.world.createdClockifyEntries).toEqual([])
        } else {
          expect(fake.world.createdClockifyEntries).toHaveLength(1)
          expect(
            new Date(fake.world.createdClockifyEntries[0]!.end ?? 0).getTime() -
              new Date(fake.world.createdClockifyEntries[0]!.start).getTime()
          ).toBe(900000)
          expect(fake.world.jiraWorklogs).toEqual([])
        }
        expect(fake.world.writtenFiles[`${FAKE_HOME}/.jcf/watch.lease`]).toBeUndefined()
      }))
  }
  ...
  it.effect("shows the issue summary without inventing assignment from a missing label", () =>
    Effect.gen(function*() {
      const { world } = yield* run(
    ...
    const policy of [
      {
        ...
        override: false,
        reason: "other-assignee"
      },
      {
        label: "another assignee with no display name",
        mode: "assigned",
        assignee: "acct-other",
        displayName: null,
        override: false,
      ...
      {
        label: "my assignment with no display name",
        mode: "assigned",
        assignee: "acct-me",
        displayName: null,
        override: false,
      ...
      { label: "ownership-any", mode: "any", assignee: "acct-other", displayName: null, override: false, reason: null },
      {
        label: "explicit ownership override",
        mode: "assigned",
        assignee: "acct-other",
        displayName: null,
        override: true,
      ...
      readonly label: string
      readonly mode: "assigned" | "any"
      readonly assignee: string | null | undefined
      readonly displayName: string | null | undefined
      readonly override: boolean
      readonly reason: "other-assignee" | "unassigned" | null
    }>
  ) {
    it.effect(`accounts for 3600 seconds under ${policy.label} without writing`, () =>
      Effect.gen(function*() {
        const { exit, world } = yield* run(
          agent(["--json"]),
          baseOptions({
            config: {
              ...
              sessionOwnership: policy.mode,
              sessionOwnershipOverrides: policy.override ? ["PROJ-5662"] : []
            },
            transcripts: {
              "work-repo/s1.jsonl": transcript({
                sessionId: "s1",
            ...
            issueSummaries: { "PROJ-5662": "Synthetic issue" },
            issueAssignees: policy.assignee === undefined ? {} : { "PROJ-5662": policy.assignee },
            issueAssigneeDisplayNames: policy.displayName === undefined ? {} : { "PROJ-5662": policy.displayName }
          })
        )
        expect(exit._tag).toBe("Success")
        const report = Option.getOrThrow(decodeAgentReport(world.stdout.join("\n")))
        expect(report.proposals.map((row) => row.sessionSeconds)).toEqual(policy.reason === null ? [3600] : [])
        expect(report.ownershipWithheld.map((row) => ({ seconds: row.sessionSeconds, reason: row.reason }))).toEqual(
          policy.reason === null ? [] : [{ seconds: 3600, reason: policy.reason }]
        )
        expect(world.createdClockifyEntries).toEqual([])
        expect(world.jiraWorklogs).toEqual([])
      }))
