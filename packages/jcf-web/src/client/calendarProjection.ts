...
export type GridBlock =
  ...
  | {
    ...
    /** Everything the row could still put in the selected providers, for the tooltip. */
...
export const projectCalendar = (
  ...
  const selectedDeltas = new Map(plan.rows.map((row) => [
    row.rowId,
    remainingSuggestionSeconds(row, plan.scope, layers)
  ]))
  const blocks = all.map((block) =>
    block.kind === "logged"
      ...
      : {
        ...block,
        deltaSeconds: selectedDeltas.get(block.rowId) ?? 0,
...
const suggestedTotal = (plan: WeekPlanResponse | null, source: "jira" | "clockify"): number => {
  ...
  return plan.rows.reduce((total, row) => {
    ...
    const credit = (row.proposal?.blocks ?? []).filter(isSuggestionBlock).reduce((sum, block) => sum + block.seconds, 0)
