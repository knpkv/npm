import { prepareProposal } from "@knpkv/jira-clockify/agent/writePlanning.js"
...
export const previewWrite = (view: WeekView, write: WeekWrite): ReadonlyArray<OptimisticEntry> => {
  ...
  return (["jira", "clockify"] satisfies Array<OptimisticEntry["source"]>).flatMap((source) => {
    const { seconds, startedAt } = sized[source]
    const startMs = startedAt?.getTime()
    return seconds > 0 && startMs !== undefined ?
      [{
        id: `optimistic:${view.entries.length}:${source}`,
        source,
...
export const settleEntries = (
  ...
  entries.flatMap((entry) => {
    const outcome = result[entry.source]
    return outcome._tag === "Written" ?
      [{
        ...entry,
        pending: false,
        endMs: entry.startMs + outcome.seconds * 1000,
