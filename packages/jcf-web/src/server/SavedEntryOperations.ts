...
import { AgentSessions, ReconcileService, Time } from "@knpkv/jira-clockify"
...
export const replaceSavedEntry = (plan: HeldPlan, entry: SavedEntry): HeldPlan => {
  ...
  while (startMs < entry.endMs) {
    // Jira accounts its whole duration on the start day; Clockify splits at local midnight.
    const endMs = entry.source === "jira" ? entry.endMs : Math.min(entry.endMs, Time.nextLocalMidnight(startMs))
    const slice = {
      entry,
      startMs,
      endMs,
      day: Time.localDay(new Date(startMs)),
      seconds: seconds({ startMs, endMs }),
...
export const validateSavedUpdate = Effect.fn("SavedEntryOperations.validate")(function*(
  ...
  if (changed && expected.source === "jira" && request.startMs < period.from.getTime()) {
    return yield* new ProposalRejectedError({
      message: "Jira worklogs belong to their start day. Choose a start within this week."
    })
  }
  if (
...
export const describeSavedEntry = Effect.fn("SavedEntryOperations.describe")(function*(options: {
  ...
  return { note: `${prefix}${summary.slice(0, 500 - prefix.length)}`, sessionCount: ids.length }
})
