...
import { ConfigService, ReconcileService, SavedEntries, SessionAttributor } from "@knpkv/jira-clockify"
import { Cache, Context, Crypto, Effect, Layer, Ref, Semaphore } from "effect"
  ...
  Effect.gen(function*() {
    ...
    return WeekPlans.of({
      ...
      keep: Effect.fn("WeekPlans.keep")(function*(plan, generation) {
        ...
        const descriptions = yield* makeRowDescriptions({ plan, config, reconcile, revision: Ref.get(revision) })
        return yield* Ref.modify(plans, (held) => {
          ...
          return [
            plan,
            [
              { plan, descriptions: previous?.descriptions ?? descriptions },
