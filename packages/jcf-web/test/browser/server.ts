...
/** Two hour-long evidence blocks per week, including their five-minute trailing presence. */
const transcriptsFor = (hours: ReadonlyArray<number>) =>
...
const makeFixture = Effect.fn("BrowserFixture.make")(function*(seed: boolean = true, savedEditing: boolean = false) {
  ...
  const fake = makeFakeHeadless({
    ...
    transcripts: transcriptsFor(savedEditing ? [9, 11, 14] : [11, 14]),
    clockifyEntries: [
      ...mondays.map((day) => ({
    ...
    jiraWorklogs: {
      "PROJ-123": mondays.map((day) => ({ started: new Date(`${day}T09:00:00`).toISOString(), timeSpentSeconds: 3600 }))
    }
  })
    ...
    Effect.gen(function*() {
      ...
      yield* router.add(
        ...
        Effect.gen(function*() {
          ...
          fixture = yield* makeFixture(seed, new URL(request.url, origin).searchParams.get("savedEditing") === "true")
