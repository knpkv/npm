...
test("fifteen-minute allocations have readable labels and do not overlap visually", async ({ page }) => {
        ...
        JSON.stringify({
          ...
          plan: {
            ...
            rows: Array.from({ length: 4 }, (_, index) => ({
              ...
              ticketKey: `RPS-${7000 + index}`,
              intervals: [],
              clockifySeconds: 0,
              jiraSeconds: 0,
              proposal: {
                ...template.proposal,
                blocks: [{ startMs: start + index * 900_000, endMs: start + (index + 1) * 900_000, seconds: 900 }],
                maxSeconds: 900,
                clockifyDelta: 900,
                jiraDelta: 900
              }
  ...
  for (let index = 0; index < 4; index++) {
    ...
    await expect(block.locator(".jcf-block-key")).toHaveText(`RPS-${7000 + index}`)
