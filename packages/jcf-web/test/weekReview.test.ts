...
it("retries a transient bootstrap failure and retains the recovered session", async () => {
  let attempts = 0
  const fixture = setup({
    bootstrapSession: async () => {
      attempts++
      if (attempts === 1) throw new RequestFailure({ status: 503, message: "Bootstrap transport failed" })
    }
  })

  await fixture.review.initialize()
  expect(fixture.state().failure).toContain("Bootstrap transport failed")
  expect(fixture.transport.readSavedWeek).not.toHaveBeenCalled()
...
it("shares one pending bootstrap between overlapping initialization attempts", async () => {
  ...
  const first = fixture.review.initialize()
  const second = fixture.review.initialize()
  expect(fixture.transport.bootstrapSession).toHaveBeenCalledTimes(1)
...
it("settles providers independently and retries only totals while retaining the saved side", async () => {
  ...
  expect(new Set(fixture.state().optimisticEntries.map((entry) => entry.id)).size).toBe(2)
...
it("keeps distinct queued cards when one provider's block has a recorded middle interval", async () => {
  const clock = timer()
  const base = fixtureWeek()
  const middle = {
    source: "clockify",
    startMs: new Date(`${base.monday}T11:20:00`).getTime(),
    endMs: new Date(`${base.monday}T11:30:00`).getTime()
  } satisfies typeof base.rows[number]["intervals"][number]
  const plan = {
    ...base,
    rows: base.rows.map((row) => ({
      ...row,
      clockifySeconds: row.clockifySeconds + 600,
      intervals: [...row.intervals, middle],
      proposal: row.proposal === undefined ? undefined : { ...row.proposal, clockifyDelta: 3000 }
    }))
  }
  const fixture = setup({
    readSavedWeek: async (week, scope) => ({ monday: week ?? plan.monday, scope, plan }),
    refreshRecordedTime: async () => plan
  }, clock.delay)
  await fixture.review.initialize()
  expect(fixture.review.queueConfirm({ rowId: "row-one", blocks: [0] })).toBe(true)
  const queued = fixture.state().queued[0]!
  expect(queued.entries.filter((entry) => entry.source === "clockify")).toHaveLength(2)
  expect(new Set(queued.entries.map((entry) => entry.id)).size).toBe(queued.entries.length)
  expect(fixture.state().optimisticEntries).toHaveLength(queued.entries.length)
  expect(fixture.review.undoQueued(queued.id)).toBe(true)
