...
it("preserves a corrected entry's source marker through Generate and Save", async () => {
  const sourceMarker = SourceConsumption.marker("2026-09-07:PROJ-5777", new Date(at(10)).getTime())
  const app = await makeApplication({
    fake: {
      writtenFiles: {},
      clockifyEntries: [{
        description: `[PROJ-5662] Corrected work\n${sourceMarker}`,
        start: at(10),
        end: at(11)
      }],
      jiraWorklogs: {},
      transcripts: {
        "repo/morning.jsonl": transcript("morning", 10),
        "repo/source.jsonl": transcript("source", 10, "PROJ-5777")
      }
    }
  })
  try {
    ...
    const generated = await app.post("/api/entries/describe", identity(plan, entry))
    expect(generated.status).toBe(200)
    const described = await Schema.decodeUnknownPromise(DescribeSavedEntryResponse)(await generated.json())
    expect(described.note).toContain(sourceMarker)
    expect(described.note?.length).toBeLessThanOrEqual(500)

    const saved = await app.post("/api/entries/update", edit(plan, entry, described.note ?? "missing"))
    expect(saved.status).toBe(200)
    expect(app.fake.world.updatedClockifyEntries[0]?.payload.description).toContain(sourceMarker)
  } finally {
    await app.web.dispose()
  }
})
...
it.each<SavedEntry["source"]>(["jira", "clockify"])(
  "keeps a corrected %s source marker when its visible description is replaced",
  async (source) => {
    const sourceStartMs = new Date(at(10)).getTime()
    const sourceEnd = new Date(new Date(at(11)).getTime() + 300000).toISOString()
    const sourceMarker = SourceConsumption.marker("2026-09-07:PROJ-5777", sourceStartMs)
    const app = await makeApplication({
      fake: {
        writtenFiles: {},
        clockifyEntries: source === "clockify"
          ? [{ description: `[PROJ-5662] Corrected work\n${sourceMarker}`, start: at(10), end: sourceEnd }]
          : [],
        jiraWorklogs: source === "jira"
          ? {
            "PROJ-5662": [{
              comment: `Corrected work\n${sourceMarker}`,
              started: at(10),
              timeSpentSeconds: 3900
            }]
          }
          : {},
        transcripts: { "repo/source.jsonl": transcript("source", 10, "PROJ-5777") }
      }
    })
    try {
      const plan = await app.read()
      const before = plan.rows.find((row) => row.ticketKey === "PROJ-5777")?.proposal
      expect(source === "clockify" ? before?.clockifyDelta : before?.jiraDelta).toBe(0)
      const entry = entryFor(plan, source)
      const visibleDescription = "Updated corrected work"

      const response = await app.post("/api/entries/update", edit(plan, entry, visibleDescription))
      expect(response.status).toBe(200)
      const saved = await Schema.decodeUnknownPromise(UpdateSavedEntryResponse)(await response.json())
      expect(saved.entry.description).toBe(`${visibleDescription}\n${sourceMarker}`)
      const providerPayload = source === "clockify"
        ? app.fake.world.updatedClockifyEntries[0]?.payload.description
        : JSON.stringify(app.fake.world.updatedJiraWorklogs[0]?.payload.comment)
      expect(providerPayload).toContain(sourceMarker)

      const refreshed = await app.read()
      const after = refreshed.rows.find((row) => row.ticketKey === "PROJ-5777")?.proposal
      expect(source === "clockify" ? after?.clockifyDelta : after?.jiraDelta).toBe(0)
    } finally {
      await app.web.dispose()
    }
  }
...
it("keeps an overnight Jira edit split when the following provider refresh fails", async () => {
  const startMs = new Date(2026, 8, 7, 23, 30).getTime()
  const app = await makeApplication({
    fake: {
      clockifyEntries: [],
      jiraWorklogs: { "PROJ-5662": [{ started: new Date(startMs).toISOString(), timeSpentSeconds: 3600 }] },
      transcripts: {}
    }
  })
  try {
    const plan = await app.read()
    const entry = entryFor(plan, "jira", startMs)
    expect(plan.rows.map((row) => [row.day, row.jiraSeconds])).toEqual([
      ["2026-09-07", 1800],
      ["2026-09-08", 1800]
    ])

    const response = await app.post("/api/entries/update", edit(plan, entry, "Updated overnight work"))
    expect(response.status).toBe(200)
    app.fake.world.jiraWorklogReadFailuresRemaining = 1
    await expect(
      readWeekStream(await app.get(`/api/week/recorded?planId=${plan.planId}`), () => {})
    ).rejects.toThrow("Jira worklog fetch failed")

    expect((await app.retained()).plan?.rows.map((row) => [row.day, row.jiraSeconds])).toEqual([
      ["2026-09-07", 1800],
      ["2026-09-08", 1800]
    ])
  } finally {
...
it("reopens consumed source time after a saved Clockify entry is shortened", async () => {
  const app = await makeApplication({
    fake: {
      clockifyEntries: [],
      jiraWorklogs: {},
  ...
  try {
    const plan = await app.read()
    const row = plan.rows.find((candidate) => candidate.proposal !== undefined)
    expect(row).toBeDefined()
    const confirmed = await app.post("/api/rows/confirm", {
      planId: plan.planId,
      rowId: row?.rowId ?? "missing"
    })
    expect(confirmed.status).toBe(200)

    const refreshed = await readWeekStream(await app.get(`/api/week/recorded?planId=${plan.planId}`), () => {})
    const entry = entryFor(refreshed, "clockify")
    const shortened = await app.post("/api/entries/update", {
      ...edit(refreshed, entry),
      endMs: entry.endMs - 1800000
    })
    expect(shortened.status).toBe(200)
    const retained = (await app.retained()).plan
    const reopened = retained?.rows.find((candidate) => candidate.rowId === row?.rowId)
    expect(reopened?.proposal?.clockifyDelta).toBe(1800)

    const remainder = await app.post("/api/rows/confirm", {
      planId: retained?.planId ?? "missing",
      rowId: reopened?.rowId ?? "missing"
    })
    expect(remainder.status).toBe(200)
    expect(app.fake.world.createdClockifyEntries).toHaveLength(2)
...
it("keeps confirmed source consumed when its Clockify entry is relabelled", async () => {
  const app = await makeApplication({
    fake: {
      clockifyEntries: [],
      jiraWorklogs: {},
  ...
  try {
    ...
    expect(row).toBeDefined()
    expect(
      (await app.post("/api/rows/confirm", {
        planId: plan.planId,
        rowId: row?.rowId ?? "missing"
      })).status
    ).toBe(200)
    ...
    const refreshed = await readWeekStream(await app.get(`/api/week/recorded?planId=${plan.planId}`), () => {})
    const entry = entryFor(refreshed, "clockify")
    expect(SourceConsumption.markers(entry.description ?? "")).toHaveLength(1)
    expect(
      (await app.post(
        "/api/entries/update",
        edit(refreshed, entry, "[PROJ-5777] Corrected ticket")
      )).status
    ).toBe(200)

    const retained = (await app.retained()).plan
    const original = retained?.rows.find((candidate) => candidate.rowId === row?.rowId)
    ...
    const rescanned = await app.read("clockify")
    expect(rescanned.rows.find((candidate) => candidate.rowId === row?.rowId)?.proposal?.clockifyDelta ?? 0).toBe(0)
    expect(app.fake.world.createdClockifyEntries).toHaveLength(1)
  } finally {
    await app.web.dispose()
  }
