...
export function SettingsTable({ items, selectedIndex }: SettingsTableProps) {
  ...
  return (
    <box style={{ flexDirection: "column", flexGrow: 1, width: "100%" }}>
      ...
      <box style={{ height: 1, width: "100%", backgroundColor: theme.backgroundElement, flexDirection: "row", paddingLeft: 1 }}>
        {isFiltering ? (
          <>
          ...
          <>
            ...
            {!settingsFilter.startsWith("on:") && !settingsFilter.startsWith("off:") ? (
              <text fg={theme.selectedText} bg={theme.primary}>{" all "}</text>
            ) : (
              <text fg={theme.textMuted}>{" all "}</text>
            )}
            {settingsFilter.startsWith("on:") ? (
              <text fg={theme.selectedText} bg={theme.primary}>{" on "}</text>
            ) : (
              <text fg={theme.textMuted}>{" on "}</text>
            )}
            {settingsFilter.startsWith("off:") ? (
              <text fg={theme.selectedText} bg={theme.primary}>{" off "}</text>
            ) : (
