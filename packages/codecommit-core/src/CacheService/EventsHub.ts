import { Context, Data, Effect, Layer, PubSub, Ref, Semaphore, Stream } from "effect"
...
const makeEventsHub = Effect.gen(function*() {
  ...
  const batchSemaphore = yield* Semaphore.make(1)
  ...
  const batch = <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
    batchSemaphore.withPermits(1)(
      Ref.set(batchingRef, true).pipe(
        Effect.andThen(Ref.set(accumulatedRef, new Set<RepoChange["_tag"]>())),
        Effect.andThen(
          Effect.ensuring(
            effect,
            Effect.gen(function*() {
