...
export const AuditLogEntry = Schema.Struct({
  ...
  permissionState: Schema.Literals(["allowed", "always_allowed", "denied", "timed_out"]),
...
export interface AuditLogRepoShape {
  ...
  }) => Effect.Effect<PaginatedAuditLog, CacheError, never>
  readonly prune: (retentionDays: number) => Effect.Effect<number, CacheError>
  readonly clearAll: () => Effect.Effect<number, CacheError>
  readonly exportAll: (opts?: {
    readonly from?: string | undefined
    readonly to?: string | undefined
...
const cacheError = (op: string) => <A, E, R>(effect: Effect.Effect<A, E, R>) =>
  effect.pipe(
    ...
    Effect.withSpan(`AuditLogRepo.${op}`)
  )

const makeAuditLogRepo = Effect.gen(function*() {
  ...
  const countAll = SqlSchema.findOne({
