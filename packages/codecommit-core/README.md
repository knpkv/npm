...
The exported `merge` action is an authority-bearing, non-idempotent direct provider mutation for an interactive client that pins `sourceCommitId`, prevents cancellation after submission, and waits for the provider receipt without interrupting the submitted call at the generic operation timeout. Its decoded target carries the captured STS caller account ID and repository-owner account ID. Immediately before dispatch, `execute` resolves both identities again; either mismatch fails before the merge provider is called. It is not a governed server-review action: Control Center deliberately excludes `merge` from its accepted action union and reconciliation locators. Other server integrations must exclude it unless they explicitly own the same non-idempotent execution and receipt-settlement lifecycle.
