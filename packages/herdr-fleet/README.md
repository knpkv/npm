...
Callers provide `HostOperations`. This keeps Nix, Git, and Herdr command execution out of the protocol package. `run` and `runCoordinatorChat` receive the accepted job ID and actor after the fleet service has enforced submission and approval. They also receive a typed lifecycle callback. Existing synchronous adapters may ignore these trailing arguments. A durable adapter supplies `recovery.matches` and `recovery.resume`, calls `accepted` once with its receipt before returning, then calls `terminal` once after the coordinator persists `settled`, `delivery_failed`, or `task_failed`. Fleet marks the job durable before invoking the adapter, stores the receipt in `acceptedReceipt`, and keeps the wrapper running between acceptance and terminal observation. If receipt validation or persistence fails after the adapter calls `accepted`, Fleet leaves the job running with a null receipt so restart recovery can idempotently rejoin the committed coordinator request. After restart, `recovery.resume` receives the deterministic job ID, either the persisted receipt or `null`, and the exact persisted worker identity or `null`. The `null` receipt case must idempotently rejoin or complete submission across a crash between the coordinator commit and Fleet receipt persistence. When a worker is present, recovery reattaches that worker and replays `workerStarted` without prompting or discovering another worker. Exact replay is accepted only during recovery, with Fleet host casing normalized; a different agent, pane, name, or relationship remains a conflict. The job ID and actor remain the idempotency and command-identity inputs.

Fleet stores a bounded terminal summary, not the coordinator's authoritative result. Pass coordinator results and failure details through `summarizeHostOperationTerminalDetail` before calling `terminal`; the authoritative full value remains in the coordinator event store. Summaries are limited to 20,000 UTF-8 bytes so successful coordinator-chat results retain the existing `CoordinatorReply` contract. The lifecycle callback validates its receipt and terminal schema before persistence, so every admitted value fits the complete Fleet record envelope.

The receipt is client-visible: Fleet stores it in both `acceptedReceipt` and `result`, and authorized raw `JobRecord` job and history responses expose both fields. Durable adapters must emit a normalized client-safe identifier. Provider callback URLs, bearer tokens, credentials, and other server-private locators are invalid receipt material. A matched durable operation that returns without calling `accepted` fails with `fleet.operation_acceptance_missing` and stays recoverable. `recovery.resume` must register observation through the hostd-owned lifetime function and return promptly; it must not wait for a terminal event, because hostd completes recovery before opening listeners.
