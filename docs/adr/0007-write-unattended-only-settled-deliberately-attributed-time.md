...
- **Only forward.** The window starts when the watch does. Work that was already over is
  `reconcile`'s job.
...
day is complete only shortly after the day is. A watch stopped mid-block loses nothing: the block was
never written, so `reconcile` still proposes it.
