...
- **One writer at a time, per machine.** Subtracting what the two sides hold makes a _later_ look
  ...
  writes it. An immutable lease in the config directory settles that.

  Three properties of that lease are load-bearing. It is won by an exclusive _create_, so two
  watches starting together cannot both read it as free. It is immutable while held: a stale-looking
  timestamp does not make read-then-overwrite takeover atomic, so an ungraceful death leaves a lock
  that requires explicit removal after verifying no watch runs. And a lease that cannot be _written_
  is not a lease held: an unwritable config directory once looked exactly like a lease already
  existing, and the watch went on to write with nothing protecting it.
  ...
  The browser uses the same file as a short-lived guard around confirmations, manual entries and
  saved-entry edits. A web mutation never publishes a watch cursor. This makes the one-writer claim
  cover every JCF provider mutation on the machine, not only two watch processes.
